...
def airquality(bot, trigger):
  ...
  if not location:
    ...
    latlong = "&latitude={}&longitude={}".format(geo_loc["lat"],geo_loc["lng"])
  location = geo_object["formatted_address"]
  results = requests.get("http://www.airnowapi.org/aq/observation/latLong/current/?format=application/json{}&distance=100&API_KEY=E585E378-3F4E-4B9C-BB71-29869684F880".format(latlong)).json()
  if not results:
