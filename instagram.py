...
instagramregex = re.compile('.*(https?:\/\/www\.instagram\.com\/p\/.+\/?).*')
...
@module.rule('.*(https?:\/\/www\.instagram\.com\/p\/.+\/?).*')
...
def parse_insta_json(json):
    ...
    try:
        ...
        icap = icap.replace('\n', ' ')
        # Truncate caption
        icap = (icap[:80] + u'…') if len(icap) > 80 else icap
    except:
        icap = False
    ...
    botmessage += u" | Likes: %s | Comments: %s" % (str(ilikes), str(icomms))
